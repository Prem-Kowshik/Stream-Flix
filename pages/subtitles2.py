...
if 'selected_video' not in st.session_state or not st.session_state.selected_video:
    ...
    with st.container():
        ...
        # Convert subtitles to base64-encoded VTT (if available)
        vtt_base64 = ""
        if st.session_state[subtitle_key]:
            vtt_data = convert_srt_to_vtt(st.session_state[subtitle_key])
            vtt_bytes = vtt_data.encode("utf-8")
            vtt_base64 = base64.b64encode(vtt_bytes).decode("utf-8")
            track_tag = f"<track label='English' kind='subtitles' srclang='en' default src='data:text/vtt;base64,{vtt_base64}' />"
        else:
            track_tag = ""

        custom_player = f"""
        <div style="background: linear-gradient(135deg, #2d2d2d, #1a1a1a); padding: 1rem; border-radius: 15px; border: 2px solid #dc2626; box-shadow: 0 8px 25px rgba(220, 38, 38, 0.3); width: 100%; max-width: 100%; box-sizing: border-box; margin: auto;">
